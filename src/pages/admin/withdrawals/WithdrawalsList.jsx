...
const WithdrawalsList = ({ status, title }) => {
  ...
  const loadWithdrawals = async () => {
    try {
      let data;
      if (status) {
        data = await withdrawalService.getWithdrawalsByStatus(status);
      } else {
        data = await withdrawalService.getAllWithdrawals();
      }
      setWithdrawals(data);
    } catch (error) {
  ...
  const handleDetailsClick = (id) => {
    navigate(`/admin/withdrawal/details/${id}`);
  };
  ...
  const filteredWithdrawals = withdrawals.filter((withdrawal) => {
    const matchesSearch = 
      ...
      withdrawal.transactionId?.toLowerCase().includes(searchQuery.toLowerCase());
    
    const matchesDateRange = 
      (!startDate && !endDate) ||
      ((!startDate || new Date(withdrawal.createdAt) >= new Date(startDate)) &&
       (!endDate || new Date(withdrawal.createdAt) <= new Date(endDate)));
                      ...
                      <tr key={withdrawal.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm">
                            <div className="text-blue-600 font-medium">{withdrawal.gateway || withdrawal.method}</div>
                            <div className="text-blue-500 text-xs">{withdrawal.transactionId}</div>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button
                            onClick={() => handleDetailsClick(withdrawal.id)}
