...
const LiveRatesWidget = () => {
  ...
  const RateCard = ({ symbol, price, change24h }) => {
    ...
    return (
      <div className="bg-[var(--bg-primary)] rounded-lg p-4 border border-[var(--border-color)] transition-colors duration-200">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[var(--text-tertiary)] text-sm font-medium">{symbol}</span>
          <button
            onClick={() => fetchRates(true)}
            disabled={refreshing}
            className="text-[var(--text-tertiary)] hover:text-[#00ADB5] transition-colors disabled:opacity-50"
          >
            <FaSync className={`w-3 h-3 ${refreshing ? 'animate-spin' : ''}`} />
          </button>
        </div>
        <div className="text-[var(--text-primary)] text-xl font-bold mb-1">
  ...
  return (
    <div className="bg-[var(--card-bg)] rounded-lg border border-[var(--border-color)] p-4 transition-colors duration-200">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-[var(--text-primary)] font-semibold">Live Rates</h3>
        <span className="text-xs text-[var(--text-tertiary)]">Updates every 5 min</span>
      </div>
